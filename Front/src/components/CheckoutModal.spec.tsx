...
describe('CheckoutModal', () => {
  ...
  it('validates required customer information', () => {
    render(<CheckoutModal product={product} quantity={1} onClose={jest.fn()} />);
  ...
  it('validates each customer and card section before preparing the payment', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Continuar' }));
    expect(screen.getByRole('alert')).toHaveTextContent('correo electrónico');

    fireEvent.change(screen.getByLabelText('Correo'), {
      target: { value: 'ana@example.com' },
    });
    fireEvent.change(screen.getByLabelText('Celular'), { target: { value: '123' } });
    fireEvent.click(screen.getByRole('button', { name: 'Continuar' }));
    expect(screen.getByRole('alert')).toHaveTextContent('celular colombiano');

    fireEvent.change(screen.getByLabelText('Celular'), { target: { value: '3001234567' } });
    fireEvent.click(screen.getByRole('button', { name: 'Continuar' }));
    expect(screen.getByRole('alert')).toHaveTextContent('datos obligatorios');
  ...
  it('completes the three checkout steps and shows an approved payment', async () => {
    ...
    fillDelivery();
    fireEvent.click(screen.getByRole('button', { name: 'Continuar' }));
    expect(screen.getByRole('heading', { name: 'Datos de la tarjeta' })).toBeVisible();
    ...
    expect(api.createCheckout).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('shows preparation errors without advancing to the summary', async () => {
    jest.mocked(paymentProvider.getAcceptance).mockRejectedValue(
